...
const SESSION_DELETE_CALLBACK_PREFIX = "session:delete:";
...
export function parseSessionIdCallback(data: string): string | null {
  ...
  if (data.startsWith(SESSION_DELETE_CALLBACK_PREFIX)) {
    return null;
  }
...
export function parseSessionDeleteCallback(data: string): string | null {
  ...
  const sessionId = data.slice(SESSION_DELETE_CALLBACK_PREFIX.length);
  return sessionId.length > 0 ? sessionId : null;
}
...
function buildSessionsKeyboard(pageData: SessionPage, pageSize: number): InlineKeyboard {
  ...
  pageData.sessions.forEach((session, index) => {
    ...
    keyboard
      .text(label, `${SESSION_CALLBACK_PREFIX}${session.id}`)
      .text(t("sessions.button.delete"), `${SESSION_DELETE_CALLBACK_PREFIX}${session.id}`)
      .row();
  });
